...
Lambda expressions are powerful and allow us to write concise, elegant, declarative code. A lot of people has already adopted them, mainly leveraging the power of streams in iterations, but lambda's toolkit is far richer than that. This is why I decided to write this blog post in order to explore some aspects of how lambda expressions can prove useful in software design.
...
Instead of creating a new case study, from scratch, I decided to extend the one from my previous blog [post](https://nvoulgaris.com/open-closed-principle-a-case-study), in which we examined closely the problem of input validation. We assumed an android application and we came up with a robust solution to validate the input of a form (leaving out android specifics on purpose and simplifying a bit to put emphasis on the right parts).
...
However, in software, there is never a *perfect* design and there will always be things to improve. As responsible, professional software craftspeople, we should always study the pros and cons of our solutions and know their weaknesses and limitations and, as I always like to say, work the trade-offs.
...
Thankfully, there is a design pattern, which solves exactly this problem and in a very elegant way: the loan pattern. I first came across it while reading Venkat Subramaniam's excellent book [Functional Programming in Java](https://www.goodreads.com/book/show/17698629-functional-programming-in-java). Venkat does an amazing job explaining this pattern by naturally refactoring a real world piece of code and I suggest that you read it [1], but I will try to explain the basic points myself here.

Essentially, the pattern removes the responsibility of dealing with the object's life cycle from the function that uses the resource. Instead, this responsibility lies with the client (the caller of the function). In order to achieve this, we *create* a resource and *pass* it to the function. As soon as the function terminates, the resource is destroyed.
...
The point is that whoever uses this piece of code does not have to worry about the life cycle of the `Validator` object. An instance will be passed as an argument and be destroyed as soon as the `validate()` function terminates. This is where the name comes from. The `Validator` object is loaned to the function.

This is a pretty *smart* and *elegant* solution to a very sneaky problem. Lambda expressions empower us to implement it.
...
So, by this point you might be wondering if it is worthwhile to go through all this refactoring. Let's take a step back and examine what we achieved. The code stills complies to the Open-Closed principle and therefore we can extend its behavior without modify it. Additionally, the responsibility of the life cycle of the `Validator` object does not lie with the caller, but we have already taken care of it. Furthermore, what's really important - and unfortunately seriously underestimated - is that the design communicates this. Finally, an additional advantage is that `Validation` instantiation logic is encapsulated in the `Validator` class.

As far as the form validation case study is concerned, it might worth it or it might not. To be honest, it doesn't matter. I did take the trouble to refactor my validation code exactly like that when I had to enhance validation after having read Venkat's book, but that is up to each one of us. Once again, as software craftspeople it's us that should work the trade-offs everyday and make the call. In any case, the important knowledge is that *we can use functional interfaces and lambda expressions to produce elegant and robust software designs*.

# Conclusion

Lambda expressions allow us to write concise and declarative code, but they offer way more than that. We can use them as a *design tool* in order to produce flexible, robust software designs. They can fundamentally change our perception of code design, especially due to the ability to pass functions as arguments. The qualities that they offer can be used to either radically shape our architecture or to enhance our micro, class level design, the latter being usually underestimated. Managing the life cycle of objects and expressing this intent via our class design can prove very useful.

* *[1] see section "Creating Fluent Interfaces Using Lambda Expressions"*
